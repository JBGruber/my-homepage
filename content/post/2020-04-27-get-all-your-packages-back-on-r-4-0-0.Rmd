...
Among the [many news](https://cran.r-project.org/doc/manuals/r-devel/NEWS.html) two stand out for me:
First, `R` now uses `stringsAsFactors = FALSE` by default, which is especially welcome when reading in data (e.g., via `read.csv`) and when constructing `data.frame`s.
...
I used a nice little piece of code I found in the [`available` package](https://github.com/r-lib/available) to find the repositories of these packages:

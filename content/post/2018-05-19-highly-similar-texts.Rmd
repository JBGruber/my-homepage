...
However, after some investigation, I found that there are indeed small differences between the articles we had identified as duplicates in our data.

`R` of course will not consider two texts duplicates when a single character, such as a comma, whitespace or quotation mark is different.
...
For some instances that could be explained by taking a look at the editions:
...
Those articles would be wrongfully counted several times in the sample.
...
For the four words above, 12 calculations are necessary, since each text needs to be calculated to each other text in the sample (i.e. n*(n-1) calculation with n = number of texts to compare).
...
dplyr::select(benchmark, mean, mem_alloc)
```

To get a feel for this, the mean time for the second calculation is `r paste(benchmark$mean[2])` that is miliseconds.
In the same unit as the first calculation, that would be `r scales::comma(as.numeric(benchmark$mean[2]) * 1000000)` microseconds.
That is an `r scales::comma(as.numeric(benchmark$mean[2]) / as.numeric(benchmark$mean[1]))`% increase on average.
...
This makes a job with hundreds or thousands of newspaper articles almost impossible to compare.
However, while the edit distance is the most accurate method for comparing texts, there are others which are much more efficient.
...
Now this is a little confusing but the lower the relative distance, the more similar the texts are---the exact opposite of the similarity measure.
What we learned from the exercise is that "Dog bites man" and "Man bites dog" have quite a relative distance from each other while the two Lorem Ipsum texts are highly similar.
In a newspaper media analysis, it would be therefore make a lot of sense to remove either text 6 or 5 from the database.

For convenience, I implemented this in the function `lnt_similarity` of my [LexisNexisTools package](https://github.com/JBGruber/LexisNexisTools#identify-highly-similar-articles).
